...
function CometChatHome(props: { theme?: string; selectedUserUuid: string; }) {
    ...
    const [selectedItem, setSelectedItem] = useState<Conversation | User | Group | Call | undefined>();
    ...
    useEffect(() => {
        ...
        if (!user) {
            setLoggedInUserError("User is not associate with any chat")
            // navigate("/login", { replace: true, state: { userUuid: props.userUuid } });
        } else {
            ...
            setUserRole(role);
            if (role === OppaiDragonUserRole.CHAT_APP.WAIFU_USER || role === OppaiDragonUserRole.CHAT_APP.ADMIN) {
    ...
    useEffect(() => {
        const isMessageListOpen =
            selectedItem &&
            (selectedItem instanceof CometChat.User || selectedItem instanceof CometChat.Group || selectedItem instanceof CometChat.Conversation);
    ...
    const TabComponent = () => {
        ...
        return <CometChatTabs onTabClicked={onTabClicked} activeTab={activeTab} userRole={userRole} />;
    };
            ...
            if (
                ((appState.selectedItemUser &&
                    (appState.selectedItemUser.getUid() == receiverId ||
                        ((!sender || (sender && appState.selectedItemUser.getUid() == sender.getUid())) && receiverId == loggedInUser?.getUid()))) ||
                    (appState.selectedItemGroup && (appState.selectedItemGroup.getGuid() == receiverId || loggedInUser?.getUid() == receiverId))) &&
            ...
            <>
                {appState.sideComponent.visible && (
                    <div className="side-component-wrapper">
                        {appState.sideComponent.type == "user" && user && <SideComponentUser user={user} userBio={props.userBio} />}
    ...
    const SideComponentUser = (props: { user: CometChat.User; userBio: string }) => {
    ...
    //         });
    //         const ccGroupMemberBanned = CometChatGroupEvents.ccGroupMemberBanned.subscribe((item: IGroupMemberKickedBanned) => {
    //             setMemberCount(item.kickedFrom.getMembersCount());
    //             setGroup(item.kickedFrom);
    //         });
    //         const ccGroupMemberKicked = CometChatGroupEvents.ccGroupMemberKicked.subscribe((item: IGroupMemberKickedBanned) => {
    //             setMemberCount(item.kickedFrom.getMembersCount());
    //             setGroup(item.kickedFrom);
    //         });
    //         return () => {
    ...
    //                                 className={`side-component-content__action-item ${appState.isFreshChat && actionItem.id === "delete_chat" ? "side-component-content__action-item-disabled" : ""} `}
    //                                 onClick={() => {
    ...
    const attachSDKGroupListener = () => {
        ...
        CometChat.addGroupListener(
            listenerId,
            new CometChat.GroupListener({
                onGroupMemberBanned: (message: CometChat.Action, kickedUser: CometChat.User, kickedBy: CometChat.User, kickedFrom: CometChat.Group) => {
                    if (
                        ((selectedItem as Group).getGuid?.() === kickedFrom.getGuid() ||
                            ((selectedItem as Conversation).getConversationWith?.() as Group)?.getGuid?.() === kickedFrom.getGuid()) &&
    ...
    const getActiveItem = () => {
        ...
        if ((activeTab === "chats" && selectedItem instanceof CometChat.Conversation) || (activeTab === "users" && selectedItem instanceof CometChat.User)) {
            return selectedItem;
        }
    ...
    useEffect(() => {
        ...
        if ((selectedItem === null || selectedItem === undefined) && loggedInUser && isMobileback !== true) {
    ...
    return (
        ...
        {loggedInUser && (
            <div
                ...
                data-theme={theme}
                // style={{
                //     height: isIphoneSafariOrChrome() ? "calc(100vh - 71px)" : "calc(100vh - 41px)",
                ...
                style={{
                    ...
                    height: "calc(100dvh - 41px - var(--safe-area-top, 0))",
                    // height: isIOS() ? 'calc(var(--vh, 1vh) * 100)' : '100vh',
                    WebkitOverflowScrolling: "touch",
                }}
            >
                ...
                >
                    <div className="selector-wrapper">
                        <CometChatSelector
                            ...
                            group={group}
                            selectedUserUuid={
                                (props.selectedUserUuid ?? userDetail.user_chat_waifus.commetchat_waifu_user?.ID)
                                    ? `users_${userDetail.user_chat_waifus.commetchat_waifu_user?.ID}`
                                    : ""
                            }
        ...
        {loggedInUserError && <div className="flex justify-center items-center h-full mt-10">{loggedInUserError}</div>}
        </>
    );
}
